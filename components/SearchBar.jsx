...
export default function AddClass() {
    ...
    const fetchData = async () => {
      try {
        ...
        if (!snapshot.empty) {
          ...
          console.log('✅ Último ID de clase:', lastClass.clase_id);
        }

        // Obtener docentes con toda su información
        ...
        const docentesList = docentesSnapshot.docs.map(doc => {
          ...
          return {
            ...
            docente_email: data.docente_email || '',
            docente_especialidad: data.docente_especialidad || '',
            ...data
          };
        });
        
        setDocentes(docentesList);
        console.log('✅ Docentes cargados:', docentesList.length);
        console.log('📋 Primer docente:', docentesList[0]);
  ...
  const handleSelectTeacher = (teacher) => {
    console.log('✅ Docente seleccionado:', teacher.docente_fullName, '- ID:', teacher.docente_id);
    setClassIdDocente(teacher.docente_id);
  };

  const handleOpenSearch = () => {
    console.log('🔍 Abriendo búsqueda con', docentes.length, 'docentes');
